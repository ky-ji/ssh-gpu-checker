...
PROCESS_QUERY = (
    ...
    "awk -F, '"
    "BEGIN { "
    "command = \"ps -eo pid=,user=\"; "
    "while ((command | getline line) > 0) { "
    "sub(/^[[:space:]]+/, \"\", line); "
    "process_pid = line; "
    "sub(/[[:space:]].*$/, \"\", process_pid); "
    "username = line; "
    "sub(/^[^[:space:]]+[[:space:]]+/, \"\", username); "
    "sub(/[[:space:]].*$/, \"\", username); "
    "users[process_pid] = username; "
    "} "
    "close(command); "
    "} "
    "{ "
    ...
    "sub(/[[:space:]]+$/, \"\", used); "
    "if (pid !~ /^[0-9]+$/) next; "
    "username = (pid in users && users[pid] != \"\" ? users[pid] : \"unknown\"); "
    "printf \"%s,%s,%s,%s\\n\", uuid, pid, used, username; "
    "}'"
)
REMOTE_QUERY = (
    "printf '__GPU__\\n'; "
